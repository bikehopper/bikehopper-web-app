...
import { getRoutes, getStops, type Route, type Stop } from 'gtfs';
...
async function appendRouteLineStringToFile(
  route: Route,
  ldGeoJsonPath: string,
  routelineLookups: RouteLinestringLookup, 
) {
...
async function appendStops(
  ...
  const getRouteTypesForStop = (stopId: string): Set<RouteTypeStrings> => {
    ...
    const routeTypes: Set<RouteTypeStrings> = new Set();
      ...
      for (const tripId of tripsForStop.values()) {
        ...
        if (routeIds && routeIds.size > 0) {
          ...
          for (const routeId of routeIds.values()) {
            ...
            const routeTypeNumber = routeTypeLookup.get(routeId);
            if (routeTypeNumber != null) {
              routeTypes.add(ROUTE_TYPE_TO_STRING[routeTypeNumber]);
            }
          }
        }
      }
...
export default async function generateRouteTiles(
  ...
  const routeTypes: Map<string, RouteTypeNumbers> = new Map();
  for (const route of routes) {
    ...
    if (!isNaN(routeType)) {
      routeTypes.set(routeId, routeType as RouteTypeNumbers);
    }
  }
